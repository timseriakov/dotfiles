...
function M:preload(job)
	...
	-- Always extract an actual video frame. The upstream preset tries to use
	-- `-map disp:attached_pic` when ffprobe reports an embedded cover, but some
	-- Matroska/Dolby Vision files make current ffmpeg reject that stream specifier
	-- with: "Invalid stream specifier: disp:attached_pic". A frame thumbnail is
	-- more useful for movies/series anyway and avoids the failing code path.
