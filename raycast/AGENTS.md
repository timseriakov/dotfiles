...
Run `raycast/fix-macked-crash.sh apply`: it builds `raycast/macked-nilguard.m` and installs it as `macked.app.dylib`. The shim swizzles that one class method process-wide, so it gates on the caller — the return address is captured inside the swizzled method itself and `dladdr()`ed, and `{}` is substituted for nil only when that address lies in the crack. Raycast and its frameworks keep stock behaviour (a nil object still raises for them). It then `dlopen()`s the untouched crack kept at `macked-orig.dylib` (also copied to `~/raycast-macked-backup/`), so the crack and the Nord theme keep working. `restore` puts the original back. Re-run `apply` after every Raycast update: it checks the installed `macked.app.dylib` for the shim marker (`NILGUARD-SHIM-1`, older builds: the `[nilguard] armed` log string) and refuses to install unless the parked crack at `macked-orig.dylib` is a real crack — when it finds a real crack installed instead (fresh repack), it rotates that file into `macked-orig.dylib` and the backup before installing the shim, so a stale crack is never loaded.
