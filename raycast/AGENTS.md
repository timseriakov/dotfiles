...
Run `raycast/fix-macked-crash.sh apply`: it builds `raycast/macked-nilguard.m` and installs it as `macked.app.dylib`. The shim swizzles that one class method process-wide, so it gates on the caller — the return address is captured inside the swizzled method itself and `dladdr()`ed, and `{}` is substituted for nil only when that address lies in the crack. Raycast and its frameworks keep stock behaviour (a nil object still raises for them). It then `dlopen()`s the untouched crack kept at `macked-orig.dylib` (also copied to `~/raycast-macked-backup/`), so the crack and the Nord theme keep working. `restore` puts the original back. Re-run `apply` after every Raycast update. When the shim is already installed it requires a real crack parked at `macked-orig.dylib` and aborts otherwise; when a real crack is installed instead (fresh repack) it rotates that file into `macked-orig.dylib` and the backup before installing the shim, so a stale crack is never loaded.

Checks: pointing `FW`/`BACKUP`/`BUILD` at a temp copy of `macked.app.dylib` exercises rotation, idempotency and `restore` without touching the live bundle; the crash path itself is visible with `log show --last 3m --predicate 'process == "Raycast"' | grep nilguard`, which should show `armed (swizzle=ok, macked=ok)` and `nil JSON object -> substituting {}`.

Two traps on this machine: `/usr/bin/pkill -x Raycast` fails with `kill: x: invalid signal specification` — use `/usr/bin/killall Raycast || true` or `osascript -e 'tell application "Raycast" to quit'`; and a pid-only `log show --predicate 'processIdentifier == N'` is insufficient across a time window — it correctly matches every process that held that pid (an `xpcproxy` row came back for the same pid), so query by process name and filter the text to `Raycast[N:`. When editing the shim, capture `__builtin_return_address(0)` inside the swizzled method itself: from a helper it reports the helper's caller (the shim), which silently disables the caller gate.
