...
(function() {
    ...
    function updateSidebar() {
        ...
        translationHistory.slice(-20).reverse().forEach((item, index) => {
    ...
    document.addEventListener('mouseup', async (e) => {
        ...
        setTimeout(async () => {
            ...
            if (selectedText && selectedText.length > 1 && selectedText.length < 1000) {
                // Check if it's not all numbers or single characters
                if (!/^[\d\s\.\,\-\+\=]+$/.test(selectedText) && selectedText.length > 2) {
    ...
    function handleKeyPress(e) {
        ...
        if (e.key === 'Escape' || e.keyCode === 27 || e.which === 27 || e.code === 'Escape') {
    ...
    console.log('Hotkeys: Space+tr (qutebrowser), F2 or Ctrl+T to toggle sidebar, ESC to close');
})();
