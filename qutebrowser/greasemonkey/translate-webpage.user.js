...
(function () {
  ...
  var existingTranslateFrame = document.querySelector(
  ...
  function updCurBtn() {
    if (curVBtn)
      curVBtn.innerText =
        (translate_toS && translate_toS + " (" + translate_to + ")") ||
        translate_to;
    toggleBtn();
  }
  ...
  if (!GM && trans && translate_to && !getCookies().googtrans)
    document.cookie = "googtrans=/auto/" + translate_to + "/; path=/";
  ...
  HTMLElement.prototype.setAttribute = function (a) {
    ...
    if (a == "aria-label") {
      ...
      if (
        err.includes("https://translate.googleapis.com/_/translate_http/_/js/")
      )
        return;
    }
        ...
        for (let i = 0, n; (n = mut.addedNodes[i]); i++) {
          if (n.nodeName == "IFRAME" && n.classList.contains("skiptranslate")) {
            ...
            if (n.classList.contains("skiptranslate")) {
              let tb = document.body.querySelector(
                "body > .skiptranslate > iframe.skiptranslate",
              );
              if (tb) {
              ...
              n.className &&
              /^[^-_]{6}-[^-_]{6}-[^-_]{6}-[^-_]{6}/i.test(n.className)
            ) {
              let s = getComputedStyle(n);
              if (s.zIndex == "1000" && s.transitionDelay == "0.6s")
                addSt("." + n.className.split(" ")[0] + " {display: none;}");
            }
          }
        }
      }
  ...
  function frameMut(f) {
    try {
      ...
      new MutationObserver(function (muts) {
        handleFrame(f);
      }).observe(f.contentWindow.document.body, {
        attributes: false,
        subtree: true,
        childList: true,
      });
  ...
  function clickIF(ev) {
    ...
    if (v == "turn_on_site") {
      ...
      if (ev.target.nodeName == "SELECT" || ev.target.nodeName == "OPTION")
        L = ev.target.value;
  ...
  function toggleMenu(t) {
    ...
    menuFrame.contentWindow.document.body.classList.toggle(
      "t_all_off",
      aBlocked,
    );
  }
  ...
  function toggleBtn() {
    ...
    if (setDefBtn) {
      setDefBtn.parentNode.classList.toggle(
        "disabled",
        curLang == translate_to,
      );
    }
  }
  ...
  function forceLang(L, A, set = true) {
    var a =
      A ||
      (langFrame &&
        langFrame.contentWindow.document.body.querySelectorAll("a")) ||
      [];
    if (!L) {
      let i,
        e,
  ...
  function handleFrame(f) {
    ...
    if (!done) {
      fixLinks(
        document.body.querySelectorAll("body > #google_translate_element a"),
      );
    }
    ...
    if (!btnAdded) {
      let btn = f.contentWindow.document.body.querySelector(
        'button[id$="restore"]',
      );
      if (btn) {
        ...
        curVBtn.onclick = function () {
          forceLang(translate_to);
        };
        toggleBtn();
        btnAdded = true;
        closeBtn =
          f.contentWindow.document.body.querySelector('a[id$="close"]');
      }
    }
  }
...
`;
  ...
  function googleTranslateElementInit() {
    ...
    el.src =
      "https://translate.google.com/translate_a/element.js?cb=googleTranslateElementInit&ok=" +
      (toolbarLang ? "&hl=" + toolbarLang : "");
    r.insertBefore(el, r.firstChild);
  }
  ...
  function locS(id) {
    ...
    if (!loc) {
      loc =
        local[gtLang] ||
        local[gtLang.split("-")[0]] ||
        local[toolbarLang] ||
        local[toolbarLang.split("-")[0]] ||
        local[navigator.language] ||
        local[navigator.language.split("-")[0]] ||
        local.en;
  ...
  const local = {
    ...
    nl: {
      _: `Schakel in vertaling voor deze site
...
Klik om te vertalen`,
    ...
    es: {
      _: `Activar la traducción para este sitio
...
Haga clic para traducir`,
    ...
    it: {
      _: `Attiva la traduzione per questo sito
...
Fai clic per tradurre`,
    ...
    de: {
      _: `Aktivieren Sie die Übersetzung für diese Site
...
Klicken Sie hier, um zu übersetzen`,
    ...
    pt: {
      _: `Ative a tradução para este site
