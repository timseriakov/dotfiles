export const SIDE_CHAT_CONFIG = {
  shortcut: "alt+;",
  shortcuts: ["alt+;", "ctrl+;", "ctrl+shift+;"],
};
...
export function patchPiSideChatOverlay(content, { replaceAny }) {
  ...
  r = replaceAny(
    ...
    [oldImports],
    newImports,
    "pi-side-chat canonical editor imports",
  );
  out = r.content;
  const oldEditorImport =
  ...
  const newEditorImport =
    'import { Key, matchesKey, truncateToWidth, visibleWidth, type Component, type Focusable, type TUI } from "@mariozechner/pi-tui";';
  r = replaceAny(
    out,
    [oldEditorImport, newEditorImport],
    newEditorImport,
    "pi-side-chat remove legacy Editor import",
  );
  out = r.content;
  r = replaceAny(
    out,
    ["  private editor: Editor;", "  private editor: CustomEditor;"],
    "  private editor: CustomEditor;",
    "pi-side-chat canonical editor type",
  );
  out = r.content;
  ...
  const newConstructor =
    "    this.editor = new CustomEditor(tui, getSideChatEditorTheme(theme), undefined, { paddingX: 0 });";
  r = replaceAny(
    out,
    [
      oldConstructor,
    ...
    newConstructor,
    "pi-side-chat canonical editor constructor",
  );
  ...
  r = replaceAny(
    out,
    [oldApiKeyResolver, newApiKeyResolver],
    newApiKeyResolver,
    "pi-side-chat use model-specific API key",
  );
  out = r.content;
...
`;
  if (!out.includes("function getSideChatEditorTheme(theme: Theme)")) {
    r = replaceAny(
      ...
      editorTheme + "export interface ForkContext {",
      "pi-side-chat editor theme adapter",
    );
    out = r.content;
  }
  ...
  r = replaceAny(
    out,
    [
      duplicateBorderConstants,
      oldBorderConstants,
      currentBorderConstants,
      whitespaceHorizontalBorderConstants,
      newBorderConstants,
    ],
    newBorderConstants,
    "pi-side-chat tmux popup frame symbols",
  );
  out = r.content;
  ...
  r = replaceAny(
    ...
    newHorizontal,
    "pi-side-chat visible horizontal frame symbols",
  );
  out = r.content;
  ...
  r = replaceAny(
    out,
    [oldTeeLeft, newTeeLeft],
    newTeeLeft,
    "pi-side-chat inward left dividers",
  );
  out = r.content;
  ...
  r = replaceAny(
    out,
    [oldTeeRight, newTeeRight],
    newTeeRight,
    "pi-side-chat inward right dividers",
  );
  out = r.content;
  const oldTop =
  ...
  if (dividerCount === 3) out = out.split(oldDivider).join(newDivider);
    ...
    throw new Error(
      `Patch 'pi-side-chat Nord dividers' expected three old dividers, found ${dividerCount}.`,
    );
  const oldBottom =
    '    lines.push(theme.fg(borderColor, "└" + "─".repeat(width - 2) + "┘"));';
  const newBottom =
  ...
  const previousCurrentPopupBody = currentPopupBody.replace("0.81", "0.9");
  const previousCurrentClosePopupBody = currentClosePopupBody.replace(
    "0.81",
    "0.9",
  );
  ...
  const tmuxPopupBody = [
    "    lines.push(theme.fg(borderColor, topLeft + horizontal.repeat(Math.max(0, width - 2)) + topRight));",
    "    const targetHeight = Math.max(3, Math.floor(this.options.tui.terminal.rows * 0.81));",
    "    const contentRows = Math.max(1, targetHeight - 2);",
    "    const editorLines = this.editor.render(innerWidth);",
    "    const maxMessageLines = Math.max(1, contentRows - editorLines.length);",
    "    for (const line of editorLines) lines.push(this.frameLine(line, innerWidth, theme, borderColor));",
  ...
  r = replaceAny(
    ...
    [
      currentPopupBody,
      currentClosePopupBody,
      previousCurrentPopupBody,
      previousCurrentClosePopupBody,
      previousTmuxPopupBody,
      tmuxPopupBody,
    ],
    currentPopupBody,
    "pi-side-chat tmux popup body",
  );
  out = r.content;
  r = replaceAny(
    out,
    ...
    "  shortcut: string;\n  shortcuts?: string[];",
    "pi-side-chat overlay shortcuts option",
  );
  out = r.content;
  const oldShortcutLabel =
    '    const shortcutLabel = this.options.shortcut.replace(/ctrl/i, "Ctrl").replace(/shift/i, "Shift").replace(/alt/i, "Alt");';
  const previousShortcutLabel =
    '    const shortcutLabel = (this.options.shortcuts ?? [this.options.shortcut]).map((shortcut) => shortcut.replace(/ctrl/i, "Ctrl").replace(/shift/i, "Shift").replace(/alt/i, "Alt")).join("/");';
  const newShortcutLabel =
    '    const displayShortcuts = (this.options.shortcuts ?? [this.options.shortcut]).filter((shortcut) => shortcut !== "ctrl+shift+;" || !(this.options.shortcuts ?? []).includes("ctrl+;"));\n    const shortcutLabel = displayShortcuts.map((shortcut) => shortcut.replace(/ctrl/i, "Ctrl").replace(/shift/i, "Shift").replace(/alt/i, "Alt")).join("/");';
  const oldShortcutInput =
    "    if (matchesKey(data, this.options.shortcut)) { this.options.onUnfocus(); return; }";
  const previousShortcutInput =
    "    if ((this.options.shortcuts ?? [this.options.shortcut]).some((shortcut) => matchesKey(data, shortcut))) { this.options.onUnfocus(); return; }";
  const closeShortcutInput =
    "    if ((this.options.shortcuts ?? [this.options.shortcut]).some((shortcut) => matchesKey(data, shortcut))) { this.dispose(); return; }";
  const requestRenderShortcutInput =
    "    if ((this.options.shortcuts ?? [this.options.shortcut]).some((shortcut) => matchesKey(data, shortcut))) { this.options.onUnfocus(); this.options.tui.requestRender(); return; }";
  const renderUnfocusShortcutInput =
    "    if ((this.options.shortcuts ?? [this.options.shortcut]).some((shortcut) => matchesKey(data, shortcut))) { this.focused = false; this.options.onUnfocus(); this.options.tui.requestRender(); return; }";
  r = replaceAny(
    out,
    [
      oldShortcutInput,
      previousShortcutInput,
      closeShortcutInput,
      requestRenderShortcutInput,
      renderUnfocusShortcutInput,
    ],
    renderUnfocusShortcutInput,
    "pi-side-chat shortcut unfocuses overlay",
  );
  out = r.content;
  const previousShortcutHints =
    "    const hints = this._focused\n      ? `${escHint} · Enter send · Alt+R refork · Alt+N clear · ${shortcutLabel} → unfocus · ${modeHint}`\n      : `${shortcutLabel} → focus side chat`;";
  const closeShortcutHints =
    "    const hints = this._focused\n      ? `${escHint} · Enter send · Alt+R refork · Alt+N clear · ${shortcutLabel} close · ${modeHint}`\n      : `${shortcutLabel} close`;";
  r = replaceAny(
    out,
    [previousShortcutHints, closeShortcutHints],
    previousShortcutHints,
    "pi-side-chat shortcut hint focuses",
  );
  out = r.content;
  r = replaceAny(
    out,
    ...
    "    this.messages.setMessages([]);",
    "pi-side-chat blank visual history",
  );
  return r.content;
}
