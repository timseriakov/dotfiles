export function createStatusLinePatches(ctx) {
    ...
    r = replaceAny(
      ...
      [
        ...
        `\t\tif (layout !== "plain-left") {\n\t\t\t// Count task jobs only until their AgentRegistry ref appears. Once it is\n\t\t\t// running, the subagent badge represents that same agent; bash and eval\n\t\t\t// jobs always remain independent background work.\n\t\t\tconst runningBackgroundJobs =\n\t\t\t\tthis.session\n\t\t\t\t\t.getAsyncJobSnapshot()\n\t\t\t\t\t?.running.filter(\n\t\t\t\t\t\tjob => job.type !== "task" || job.agentId === undefined || !this.#runningSubagentIds.has(job.agentId),\n\t\t\t\t\t).length ?? 0;\n\t\t\tif (runningBackgroundJobs > 0) {\n\t\t\t\tconst count = placeholders ? "…" : ` +
          ...
          `;\n\t\t\t\trightParts.unshift(theme.fg("statusLineSubagents", ` +
          "`${theme.icon.job} ${count}`" +
  ...
  function patchSegments(content) {
    ...
    const upstreamGitWithStatusValue =
    ...
    const upstreamSessionName18_1_10 =
    ...
    r = replaceAny(
      ...
      `const VIM_MODE_LABELS: Record<NonNullable<SegmentContext["vim"]>["mode"], string> = {\n\tinsert: "I",\n\tnormal: "N",\n\tvisual: "V",\n\t"visual-line": "VL",\n};`,
      "segments compact vim mode labels",
    );
    out = r.content;
